...
public class EcoreEnvironmentFactory
		extends
		AbstractEnvironmentFactory<EPackage, EClassifier, EOperation, EStructuralFeature, EEnumLiteral, EParameter, EObject, CallOperationAction, SendSignalAction, Constraint, EClass, EObject> {
	 ...
	 * specified to lookup up and navigate hidden opposites on references based
	 * on annotations.
	 * 
	 * @since 3.1
	 */
	 ...
	 * for the non-existing opposite's name a
	 * <code>DefaultOppositeEndFinder</code> will be used. To not look for those
	 * annotations at all, use
	 * {@link #EcoreEnvironmentFactory(org.eclipse.emf.ecore.EPackage.Registry, OppositeEndFinder)}
	 * and pass <code>null</code> for the <code>OppositeEndFinder</code>
	 * argument.
	 * 
	 * @param reg
	 ...
	 * <code>OppositeEndFinder</code> that the environments I create will use to
	 * construct and evaluate <code>OppositePropertyCallExp</code> expressions
	 * based on name annotations on the forward references.
	 * 
	 * @param reg
	 *            my package registry (must not be <code>null</code>)
	 * @param finder
	 *            for opposite ends; may be <code>null</code> which means that
	 *            no <code>OppositePropertyCallExp</code> expressions will be
	 *            synthesized because no lookups can/will be performed for them.
	 *            To use a default finder, use
	 *            {@link #EcoreEnvironmentFactory(org.eclipse.emf.ecore.EPackage.Registry)}
	 *            or {@link #EcoreEnvironmentFactory()}.
	 * @since 3.1
	 */
	public EcoreEnvironmentFactory(EPackage.Registry reg,
