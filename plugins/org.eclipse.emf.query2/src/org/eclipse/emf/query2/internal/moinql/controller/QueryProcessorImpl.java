...
public class QueryProcessorImpl implements QueryProcessor {
	 ...
	 */
	private PreparedQuery prepareInternal(Query query, ProcessReport report, QueryContext context, EmfHelper emfHelper)
		...
		try {
			...
			if (logger.isTraced(LogSeverity.INFO)) {
				logger
						.trace(LogSeverity.INFO, FQLTraceMessages.MQL_PROCESSOR_FURTHER_PRE_PROCESSING_OF_INTERNAL_QUERY,
			...
			if (logger.isTraced(LogSeverity.WARNING)) {
				if (report.getProcessStatus() == ProcessReport.SUCCESSFUL_WITH_WARNINGS) {
					...
					logger.trace(LogSeverity.WARNING,
							FQLTraceMessages.MQL_PROCESSOR_FOUND_WARNINGS_DURING_PREPARATION_OF_THE_FOLLOWING_QUERY,
							"\n", (internalQuery == null ? "NULL query" : internalQuery)); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private ResultSet executeSecuredInternal(PreparedQuery preparedQuery, EmfHelper emfHelper, boolean schedulingWanted,
		...
		try {
			...
			try {
				...
				internalQuery = scheduler.schedule(internalQuery, schedulingWanted, globalScopeIncluded, globalPartitionScope,
						globalContainerScope, this.thresholdForNumberOfRelevantPartitionsForOptimization,
						this.thresholdForNumberOfElementsInPartitionForOptimization);
	 ...
	 */
	private ResultSet executeInternal(Query selectQuery, QueryContext context, boolean schedulingWanted, URI[] globalContainerScope,
			int numberOfResults) throws QueryExecutionException, QueryFormatException {
	...
	public ResultSet execute(Query query, QueryContext scopeProvider, int numberOfResults) throws QueryExecutionException,
	...
	public ResultSet execute(String query, QueryContext scopeProvider, int numberOfResults) throws QueryExecutionException,
			QueryFormatException {
