...
//		assureMetamodelsLoaded();
		...
		if (input instanceof IAdaptable) {
			...
			if (null != (obj = ((IAdaptable) input).getAdapter(RefPackage.class))) {
				RefPackage root = (RefPackage) obj; // JMIReflection.MMBridge.getRoot((RefPackage)
													// obj,MofPackage.class.getName());
				return root;
			}
		}
		if (input instanceof RefPackage) {
			RefPackage root = (RefPackage) input; // JMIReflection.MMBridge.getRoot((RefPackage)input,MofPackage.class.getName());
			return root;
		}
		return input;
	}
	...
	public static RefObject getBusinessRefObject(Object object) {
		//		
		RefObject result = (RefObject) Platform.getAdapterManager().getAdapter(object, RefObject.class.getName());
