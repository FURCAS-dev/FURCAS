		...
		try {
			...
			if (obj instanceof GeneralizableElement) {
				...
				if (status.isOK()) {
					...
					if (!result.isOK()) {
						sTracer.log(Level.SEVERE, "Exception while fixing constraint violation: " + result.getMessage(), //$NON-NLS-1$
								result.getException());
					} else {
			...
			sTracer.log(Level.SEVERE, "Exception while fixing constraint violation:", e); //$NON-NLS-1$
