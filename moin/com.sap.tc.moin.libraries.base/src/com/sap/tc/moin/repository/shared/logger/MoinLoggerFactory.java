...
import com.sap.tc.moin.repository.shared.logger.impl.MoinLoggerImpl;
...
public class MoinLoggerFactory {
     ...
     * </ol>
     * 
     * @param category the logical-defined Category name
     * @param location the package related Location name
     * @param subLocation the sub-location which allows you the class-oriented
     * message localization
     * @return ready-to-use MoinLogger
     */
    public static synchronized MoinLogger getLogger( MoinCategoryEnum category, MoinLocationEnum location, Class subLocation ) {
        ...
        return new MoinLoggerImpl( log, trace, subLocationName );
    }

    // The already used / instantiated Categories
    private static Hashtable<String, Category> logs = new Hashtable<String, Category>( );

    // The already used / instantiated Locations
    private static Hashtable<String, Location> traces = new Hashtable<String, Location>( );
    ...
    public static void setAllLocationsToDebug( ) {
        ...
        Location locMoin = getTraceObject( MoinLocationEnum.MOIN.getLocationName( ) );
    ...
    static {
        ...
        throwRuntimeException = Boolean.valueOf( System.getProperty( "com.sap.tc.moin.errorAsRuntimeException", "false" ) ).booleanValue( ); //$NON-NLS-1$ //$NON-NLS-2$

        //buildUpLoggerNames( );
        String categoryName = null;
        for ( MoinCategoryEnum categoryEnum : MoinCategoryEnum.values( ) ) {
            controllers.put( categoryEnum.toString( ), categoryEnum.getCategoryDescription( ) );
        }

        String locationName = null;
        for ( MoinLocationEnum locationEnum : MoinLocationEnum.values( ) ) {
