...
public abstract class SpiAbstractBasicQueryProcessor implements SpiBasicQueryProcessor {
    ...
    protected void checkQueryConsistency( SpiSelectExpression selectExpression ) {
        ...
        this.checkModelElementClusterConsistency( selectExpression.getModelElementClusterExpression( ), new HashSet<SpiModelElementClusterExpression>( ) );
     ...
     * 
     * @param selectExpression select expression to eb checked
     */
    private void checkSelectListModelElementsExpressionConsistency( SpiSelectExpression selectExpression ) {
        ...
        if ( selectLists.length == 0 ) {
            throw new SpiFacilityQueryServiceException( SpiFacilityQueryServiceException.ERROR_CODE_UNSPECIFIED, BQLBugMessages.SELECTEXPRESSION_MUST_HAVE_AT_LEAST_ONE_SELECTLIST_SELECTEXPRESSION, selectExpression );
        }
        for ( int numSelList = 0; numSelList < selectLists.length; numSelList++ ) {
            ...
            if ( !this.isModelElementExpressionInModelElementClusterExpression( modelElementExpressionOfSelectList, modelElementClusterExpression ) ) {
                throw new SpiFacilityQueryServiceException( SpiFacilityQueryServiceException.ERROR_CODE_UNSPECIFIED, BQLBugMessages.MODELELEMENTEXPRESSIONS_IN_SELECTLISTS_MUST_BE_PART_OF_THE_ROOT_MODELELEMENTCLUSTEREXPRESSION );
            }
        }
    }
     ...
     * otherwise false
     */
    private boolean isModelElementExpressionInModelElementClusterExpression( SpiModelElementExpression modelElementExpression, SpiModelElementClusterExpression modelElementClusterExpression ) {

        SpiModelElementExpression headerModelElementExpression = modelElementClusterExpression.getHeaderModelElementExpression( );
        List<SpiModelElementExpression> additionalModelElementExpressions = modelElementClusterExpression.getAdditionalModelElementExpressions( );
     ...
     * in the same cluster.
     * 
     * @param attributeExpression attribute expression
     * @param modelElementClusterExpression model element cluster expression
     */
    private void checkAttributeExpressionInClusterConsistency( SpiAttributeExpression attributeExpression, SpiModelElementClusterExpression modelElementClusterExpression ) {
        ...
        if ( attributeExpression instanceof SpiAttributeToAttributeComparisonExpression ) {
            ...
            SpiAttributeInModelElement comparedAttributeInModelElement = attributeToAttributeComparisonExpression.getComparedAttributeInModelElement( );
            if ( !this.isModelElementExpressionInModelElementClusterExpression( comparedAttributeInModelElement.getModelElementExpression( ), modelElementClusterExpression ) ) {
                StringBuffer sb = new StringBuffer( );
                attributeExpression.toString( sb, 0, false );
                throw new SpiFacilityQueryServiceException( SpiFacilityQueryServiceException.ERROR_CODE_UNSPECIFIED, BQLBugMessages.ATTRIBUTE_CANNOT_BE_COMPARED_TO_AN_ATTRIBUTE_IN_ANOTHER_CLUSTER_ATTRIBUTE_EXPRESSION, sb );
            }
        }
    }
     ...
     * @param modelElementClusterExpression
     */
    private void checkModelElementClusterConsistency( SpiModelElementClusterExpression modelElementClusterExpression, Set<SpiModelElementClusterExpression> checkedClusters ) {

        // check whether this cluster has been checked before
        if ( checkedClusters.contains( modelElementClusterExpression ) ) {
            throw new SpiFacilityQueryServiceException( SpiFacilityQueryServiceException.ERROR_CODE_UNSPECIFIED, BQLBugMessages.FOUND_LOOP_IN_THE_HIERARCHY_OF_THE_MODEL_ELEMENT_CLUSTER_EXPRESSIONS, modelElementClusterExpression.getHeaderModelElementExpression( ).getAlias( ) );
        }
        ...
        List<SpiModelElementExpression> additionModelElementExpressions = modelElementClusterExpression.getAdditionalModelElementExpressions( );
    ...
    private void checkModelElementExpressionConsistency( SpiModelElementExpression modelElementExpression ) {
        ...
        if ( modelElementExpression.getElements( ) != null ) {
            ...
            // no partition expression set
                ...
                throw new SpiFacilityQueryServiceException( SpiFacilityQueryServiceException.ERROR_CODE_UNSPECIFIED, BQLBugMessages.MODELELEMENTEXPRESSION_INCONSISTENCY_TYPES_AND_PARTITIONEXPRESSION_SET_NOT_ALLOWED_IN_CASE_ELEMENTS_ARE_SET );
            }
     ...
     */
    abstract public SpiFqlQueryResultSet execute( SpiFacilityQueryClientScope queryClientScope, SpiSelectExpression selectExpression, int maxResultSetSize, int numberOfRequestedRows );
    ...
    public SpiFqlQueryResultSet execute( SpiFacilityQueryClientScope queryClientScope, SpiFqlQuery iqlQuery, int maxResultSetSize, int numberOfRequestedRows ) {
        ...
        throw new MoinLocalizedBaseRuntimeException( BQLBugMessages.FQLDEPRECATED );
    }
