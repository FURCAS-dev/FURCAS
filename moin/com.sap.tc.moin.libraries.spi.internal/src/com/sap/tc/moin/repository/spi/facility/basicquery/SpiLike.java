...
public final class SpiLike extends SpiLeafExpression {
        ...
        for ( int currentPos = 0; currentPos < patternLength; currentPos++ ) {
            ...
            switch ( currentChar ) {
                ...
                case ESCAPE_CHARACTER:
                    addWildcardCharacterIntegerObjectToParseResultList( numberOfOneArbitraryCharacters, numberOfZeroToNArbitraryCharacters, parseResultList );
                    numberOfOneArbitraryCharacters = 0;
                    numberOfZeroToNArbitraryCharacters = 0;
                    currentPos++;
                    if ( currentPos >= patternLength ) {
                        throw new SpiFacilityQueryServiceException( SpiFacilityQueryServiceException.ERROR_CODE_UNSPECIFIED, BQLBugMessages.FALSE_ESCAPE_CHARACTER_SETTING_PATTERN, _pattern );
                    }
     ...
     * if required
     * 
     * @param numberOfOneArbitraryCharacters number of special characters
     * specifying one arbitrary character
     * @param numberOfZeroOneToNArbitraryCharacters number of special characters
     * specifying zero to n arbitrary characters
     * @param parseResultList parse result list which is to be updated
     */
    private void addWildcardCharacterIntegerObjectToParseResultList( int numberOfOneArbitraryCharacters, int numberOfZeroToNArbitraryCharacters, List<Object> parseResultList ) {
