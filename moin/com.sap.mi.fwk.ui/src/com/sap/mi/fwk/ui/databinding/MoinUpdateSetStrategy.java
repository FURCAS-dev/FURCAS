 ...
 * observable sets}. The following behaviors can be customized via the strategy:
 * <ul>
 * <li>Conversion</li>
 ...
 * 
 * 
 * @see MoinDataBindingContext#bindSet(IObservableSet, IObservableSet,
 *      UpdateSetStrategy, UpdateSetStrategy)
 * @see IConverter
 * @author d027044
*/
public final class MoinUpdateSetStrategy extends UpdateSetStrategy {

	/**
	 * Creates a new MOIN update set strategy for automatically updating the
	 * destination observable set whenever the source observable set changes. A
	 * default converter will be provided. The defaults can be changed by
	 * calling one of the setter methods.
	 */
	public MoinUpdateSetStrategy() {
		this(true, POLICY_UPDATE);
	}

	/**
	 * Creates a new MOIN update set strategy with a configurable update policy.
	 ...
	 *            {@link #POLICY_UPDATE}
	 */
	public MoinUpdateSetStrategy(int updatePolicy) {
		this(true, updatePolicy);
	}

	/**
	 * Creates a new MOIN update set strategy with a configurable update policy.
	 * A default converter will be provided if <code>provideDefaults</code> is
	 * <code>true</code>. The defaults can be changed by calling one of the
	 * setter methods.
	 * 
	 * @param provideDefaults
	 *            if <code>true</code>, default validators and a default
	 *            converter will be provided based on the observable set's type.
	 * @param updatePolicy
	 *            one of {@link #POLICY_NEVER}, {@link #POLICY_ON_REQUEST}, or
	 *            {@link #POLICY_UPDATE}
	 */
	public MoinUpdateSetStrategy(boolean provideDefaults, int updatePolicy) {
	 ...
	 */
	@Override
	public MoinUpdateSetStrategy setConverter(IConverter converter) {
		return (MoinUpdateSetStrategy) super.setConverter(converter);
	}
}
