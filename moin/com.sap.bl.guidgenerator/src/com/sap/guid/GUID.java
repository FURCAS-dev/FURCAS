...
public class GUID implements Serializable, IGUID, Comparable {
		...
		if ( (forceNativeLibrary && ! IsNativeLibraryAvail) ||
		     (forceMACAddress && ! IsHardwareMACAvail) )
    	      	...
    	      	throw new MissingResourceException( 
	              		"Missing ressource: " + PerfLibException.getMessage(),GUID.class.getName(), "ieee802.address");
    ...
    private static long getnodeaddress() {
        ...
        //TODO use different library for this?
...
//		}
	...
	public int compareTo(Object otherGUID) {
		if (otherGUID == null) {
			throw new IllegalArgumentException( "otherGUID must not be null" );	
		}	
		if (! (otherGUID instanceof GUID ||
		        otherGUID instanceof byte[]) ) {
		    throw new IllegalArgumentException(
		    	"otherGUID must be of type " + otherGUID.getClass().getName() );
		}
		GUID other = null;
		if (otherGUID instanceof byte[]) {
			try {
				...
				throw new IllegalArgumentException(
					"Malformed parameter: otherGUID", e);
			}
		}
		else {
