...
public class PlatformServiceImpl implements IPlatformService {
	...
	public String askString(String title, String message, String oldString) {
		...
		InputDialog inputDialog = new InputDialog(shell, title, message, oldString, null);
	...
	public Color editColor(Color color) {
		if (color != null && color.refImmediateComposite() instanceof Diagram) {
			...
			Color newColor = GaUtil.manageColor(diagram, retRgb.red, retRgb.green, retRgb.blue);
			return newColor;
	...
	public ColoredFont editFont(AbstractText text, ColoredFont coloredFont, Diagram diagram) {
	...
	public Object[] objectSelection(String title, String message, Object[] selectableElements, Object[] selectedElements,
			boolean singleSelection, ILabelProvider labelProvider) {
		ObjectSelectionDialog osd = new ObjectSelectionDialog(GraphicsPlugin.getShell(), title, message);
		...
		osd.setLabelProvider(eclipseLabelProvider);

		Object selectedObjects[] = osd.openDialog(selectableElements);
	...
	private class LabelProviderAdapter extends LabelProvider {
		...
		public Image getImage(Object element) {
			...
			String imageIdFromDelegate = getLabelProviderDelegate().getImageId(element);
			if (imageIdFromDelegate != null) {
				Image imageFromPool = ImagePool.getImageForId(imageIdFromDelegate);
		...
		public String getText(Object element) {
			...
			String textFromDelegate = getLabelProviderDelegate().getText(element);
		...
		private void setLabelProviderDelegate(ILabelProvider labelProviderDelegate) {
	...
	public boolean doFixes(RefObject[] objects) {
		...
		for (RefObject bo : objects) {
			if (bo != null) {
				IMarker[] markersFound = MarkerManager.getInstance().findMarkers(bo, IMarker.PROBLEM, true, IResource.DEPTH_INFINITE);
	...
	public IDiagramEditor createDiagramEditorDummy(IDiagramTypeProvider diagramTypeProvider) {
		DiagramEditorDummy diagramEditor = new DiagramEditorDummy(diagramTypeProvider);
