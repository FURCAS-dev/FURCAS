...
/**
 * This ContextMenuProvider provides a standard-context-menu and adds it to the given EditPartViewer. Another feature is, that it
 * can set the menu-location to all Actions, which implement the interface ILocationReceiver.
 * <p>
...
public class GraphicsContextMenuProvider extends ContextMenuProvider {
    ...
    // currently not needed anymore; see avoidance of OS.GetCurrentPos() below
     ...
     *            The control which serves as origin to calculate the menu-location. If null, then the menu-location is not
	...
	/*
	 * trying to avoid OS.GetMessagePos() Point menuLocation = getCurrentMouseLocation(); addDefaultMenuGroupSave(manager,
	 * menuLocation); addDefaultMenuGroupEdit(manager, menuLocation); addDefaultMenuGroupPrint(manager, menuLocation);
	 * addDefaultMenuGroupRest(manager, menuLocation);
	 */
	addDefaultMenuGroupSave(manager, mouseLocation.getSWTPoint());
	addDefaultMenuGroupEdit(manager, mouseLocation.getSWTPoint());
	addDefaultMenuGroupPrint(manager, mouseLocation.getSWTPoint());
	addDefaultMenuGroupRest(manager, mouseLocation.getSWTPoint());
    }
	...
	if (pes.length == 1) {
	    extendCustomContext(pes[0], (CustomContext) context, menuLocation);
	}
    ...
    private void addEntries(IMenuManager manager, IContextMenuEntry[] contextMenuEntries, Point menuLocation,
	    ICustomContext context, String groupID, String textParentEntry) {
    ...
    private void extendCustomContext(PictogramElement pe, CustomContext context, Point location) {
	/*
	 * avoid OS.GetCurrentPos() and pass location as parameter instead Point location = getCurrentMouseLocation();
	 */
    ...
    /**
     * Returns the current mouse menuLocation with regard to the reference-control. Returns null, if the mouse menuLocation is
     * outside the area of the reference-control.
     * 
     * @return The current mouse menuLocation with regard to the reference-control.
     */
    /*
     * refactored all usages to use an already existing mouse location instead; hope it works; can't currently test; in case it
     * doesn't, well, then we'd need this piece of platform-specific code again, but I hope not.
     */
    /*
     * protected Point getCurrentMouseLocation() { if (_menuLocationReferenceControl != null) { int pos = OS.GetMessagePos(); int
     * x = (short) (pos & 0xFFFF); int y = (short) (pos >> 16); Point point = new Point(x, y); Point location =
     * Display.getCurrent().map(null, _menuLocationReferenceControl, point); Rectangle rectangle = new Rectangle(0, 0,
     * _menuLocationReferenceControl.getSize().x, _menuLocationReferenceControl .getSize().y); if (rectangle.contains(location))
