...
public class UpdateAssociationEndNameFeature extends UpdateTextDecoratorFeature{

    public UpdateAssociationEndNameFeature(IFeatureProvider fp) {
        super(fp);
    }

	@Override
	protected String getLinkPropertyValue() {
		return MOFToolConstants.ASSOCIATION_END_NAME;
	}
	...
	public static int calculateOffsetY(ConnectionDecorator decorator, Text text) {
		...
		if (startY < endY) {
			...
			if (decorator.getLocation() > 0.5) {
				...
				offsetY = 0 - (GaUtilInternal.getFont(text, true).getSize() * 2);
			}
		}
		else {
			// Inverse orientation (bottom to top)
			if (decorator.getLocation() < 0.5) {
				// This is the first end (bottom)
				// TODO use real height of text field
				offsetY = 0 - (GaUtilInternal.getFont(text, true).getSize() * 2);
			}
