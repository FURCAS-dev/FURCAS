...
public class HashBag implements Bag {
    ...
    private final Map<OclAny, Integer> elementCountMap = new HashMap<OclAny, Integer>( );
     ...
     * 
     * @param c The collection to populate the bag with.
     */
    public HashBag( Collection<OclAny> c ) {
     ...
     * 
     * @param o The entry to count.
     * @return The number of times o appears in the bag, or 0 if it is not
     * there.
     */
     ...
     * 
     * @param c The collection containing the objects to add.
     * @return <code>true</code> when the bag is changed. This is only false
     * if c is empty.
     */
    public boolean addAll( Collection<? extends OclAny> c ) {

        Iterator<? extends OclAny> i = c.iterator( );
        while ( i.hasNext( ) ) {
            add( i.next( ) );
        }
        return !c.isEmpty( );
     ...
     * 
     * @return <code>true</code> if there are one or more instances of o in
     * this bag.
     */
    ...
    /**
     * Tests if the bag is equal to another {@link Bag}. The other Bag need not
     * be of this class, but it must meet the same semantics.
     * 
    ...
    public boolean remove( Object o ) {
        ...
        if ( intValue == 0 ) {
            ...
            if ( o instanceof OclAny ) {
                OclAny oa = (OclAny) o;
                this.elementCountMap.put( oa, Integer.valueOf( intValue ) );
            }
        }
        return true;
    }
    ...
    /**
     * Tests if this bag contains all the elements in another collection. The
     * number of elements in this bag and the collection is not considered.
     * 
     * @return <code>true</code> if every element of <em>a</em> is found in
     * this bag.
     */
    ...
    /**
     * Removes all elements from <em>a</em> from this bag. At the end of this
     * operation the bag will not contain any elements that are in <em>a</em>.
     * 
    ...
    /**
     * Removes all entries which are not found in <em>a</em>. Duplicates in
     * this bag which only appear once in <em>a</em> will be left alone.
     * 
     * @return <code>true</code> if this bag was modified.
     */
    public boolean retainAll( Collection<?> a ) {
        ...
        boolean changed = false;
        Iterator<OclAny> i = this.elementCountMap.keySet( ).iterator( );
        while ( i.hasNext( ) ) {
            Object key = i.next( );
            if ( !a.contains( key ) ) {
                i.remove( );
    ...
    public Iterator<OclAny> iterator( ) {
        ...
        return new BagIterator( this.elementCountMap.entrySet( ).iterator( ) );
    }
     ...
     */
    private ArrayList<OclAny> toArrayList( ) {

        ArrayList<OclAny> array = new ArrayList<OclAny>( size( ) );
        Iterator<Map.Entry<OclAny, Integer>> i = this.elementCountMap.entrySet( ).iterator( );
        while ( i.hasNext( ) ) {
            Map.Entry<OclAny, Integer> entry = i.next( );
            OclAny key = entry.getKey( );
    ...
    /**
     * An {@link java.util.Iterator} implementation to return the elements of
     * the current bag.
     */
    private static final class BagIterator implements Iterator<OclAny> {

        // an iterator for the internal map
        private Iterator<Map.Entry<OclAny, Integer>> myElementCountMapIterator;

        // The current entry in the map
        private Map.Entry<OclAny, Integer> entry;
        ...
        /**
         * Default constructor. Picks up an iterator from the map in the
         * encapsulating class.
         */
        BagIterator( Iterator<Map.Entry<OclAny, Integer>> elementCountMapIterator ) {

            this.myElementCountMapIterator = elementCountMapIterator;
            this.entry = null;
            this.entrySize = 0;
        ...
        public void remove( ) {
            ...
            if ( --this.entrySize == 0 ) {
                this.myElementCountMapIterator.remove( );
            } else {
        ...
        public boolean hasNext( ) {
            ...
            return this.entryPosition < this.entrySize || this.myElementCountMapIterator.hasNext( );
        }
        ...
        public OclAny next( ) {
            ...
            if ( ++this.entryPosition > this.entrySize ) {
                this.entry = this.myElementCountMapIterator.next( );
