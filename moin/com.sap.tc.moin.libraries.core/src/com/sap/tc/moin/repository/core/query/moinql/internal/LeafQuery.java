...
import com.sap.tc.moin.repository.spi.facility.fql.SpiFqlAttributeSelection;
...
import com.sap.tc.moin.repository.spi.facility.fql.SpiFqlSelection;
...
final public class LeafQuery extends InternalQuery implements SpiFqlQuery {
     ...
     * Use factory method!
     */
    private LeafQuery( AliasName _aliasName, List<AtomicEntry> _fromEntries, List<WithEntry> _withEntries, List<LeafSelectEntry> _selectEntries ) {
     ...
     */
    static public LeafQuery construct( AliasName _aliasName, List<AtomicEntry> _fromEntries, List<WithEntry> _withEntries, List<LeafSelectEntry> _selectEntries ) {
     ...
     * assoc)
     */
    static public LeafQuery construct( AliasName _aliasName, List<AtomicEntry> _fromEntries, List<WithEntry> _withEntries, List<LeafSelectEntry> _selectEntries, SpiFacilityQueryLanguage _fqlProcessor ) {
    ...
    public String toString( int indent, StringBuilder accumSb ) {
        ...
        // select Entries
        String bqProcessorName = MQLAuxServices.EMPTYSTR;
        if ( this.fqlProcessor != null ) {
            bqProcessorName = MQLAuxServices.OPENBRACKET_T + this.fqlProcessor.getFacilityId( ) + MQLAuxServices.CLOSEBRACKET_T;
        }

        String selectEntriesPrefix = MQLAuxServices.OPENPAREN_T + MQLAuxServices.SELECT_T + bqProcessorName + MQLAuxServices.SPACE_T;
        sb.append( selectEntriesPrefix );

        if ( this.selectEntries.size( ) == 0 ) {
            throw new MQLBugException( MQLBugMessages.NO_SELECT_ENTRIES );
        } else {
        ...
        if ( this.fromEntries.size( ) == 0 ) {
            throw new MQLBugException( MQLBugMessages.NO_FROM_ENTRIES );
        } else {
    ...
    public SpiFqlSelection[] getQuerySelections( ) {
        ...
        if ( this.selections == null ) {
            ...
            throw new MQLBugException( MQLBugMessages.SETSELECTIONS_ORDERING_WRONG );
        }
    ...
    public void setSelections( Map<SelectEntry, PositionRecord> positionMap ) {
        ...
        for ( LeafSelectEntry selectEntry : this.selectEntries ) {
            ...
            if ( typeRef instanceof AtomicAttrReference ) {
                ...
                FqlAttributeSelectionImpl attributeSelection = new FqlAttributeSelectionImpl( typeAttrRef.getAttrName( ), typeAttrRef.getAttrType( ), typeAttrRef.isMultiValued( ) );
