...
import org.eclipse.emf.query.index.impl.query.QueryUtil;
import org.eclipse.emf.query.index.internal.ResourceDescriptorInternal;
import org.eclipse.emf.query.index.maps.ListMap;
import org.eclipse.emf.query.index.maps.SingleMap;
import org.eclipse.emf.query.index.maps.SerializationStrategy.Channel;
import org.eclipse.emf.query.index.util.FilteredIterable;
...
public class PageableResourceDescriptorImpl implements ResourceDescriptorInternal {
	...
	private ListMap<String, IncomingReferenceDescriptorImpl> incomingLinkTable;
	...
	private void initIncomingContentTables() {
		incomingLinkTable = new ListMap<String, IncomingReferenceDescriptorImpl>(IncomingReferenceDescriptorImpl.TARGET_FRAGMENT, 2);
	}
	...
	public void addIncomingLink(ReferenceDescriptorImpl incomingLink, PageableResourceDescriptorImpl fromResDesc) {
		...
		if (eObjectTable == null || (targetDesc = eObjectTable.getEqual(key)) == null) {
			IncomingReferenceDescriptorImpl exRef = incomingLinkTable.getEqual(key);
			if (exRef != null) {
		...
		Collection<IncomingReferenceDescriptorImpl> incLinks = this.incomingLinkTable.getAllWithEqualKey(key);
		for (IncomingReferenceDescriptorImpl incLink : incLinks) {
			if (incLink.getSourceFragment() == incomingLink.getSourceFragment() && incLink.getSourceResourceURI() == fromResDesc.getURI()) {
				return;
			}
	...
	public void removeIncomingLink(ReferenceDescriptorImpl incomingLink, PageableResourceDescriptorImpl fromResDesc) {
		...
		for (Iterator<IncomingReferenceDescriptorImpl> it = this.incomingLinkTable.iterator(incomingLink.getTargetFragment()); it.hasNext();) {
			IncomingReferenceDescriptorImpl extRefDesc = it.next();
		...
		case BACKWARD:
			if (tgtFragment == null || tgtFragment.indexOf('*') > -1) {
				...
				return new FilteredIterableMulti<ReferenceDescriptorImpl>() {
					...
					Iterator<IncomingReferenceDescriptorImpl> scopeIterator;
					private IncomingReferenceDescriptorImpl next;
				...
				final Iterator<IncomingReferenceDescriptorImpl> scopeIterator = this.incomingLinkTable.getAllWithEqualKey(tgtFragment)
						.iterator();
				...
				return new FilteredIterableMulti<ReferenceDescriptorImpl>() {
					...
					protected Iterator<? extends ReferenceDescriptorImpl> getNextIterator() {
						while (scopeIterator.hasNext()) { // FIXME same loop as
							// above
							IncomingReferenceDescriptorImpl next = scopeIterator.next();
							if (refQuery.getSourceScope() == null || refQuery.getSourceScope().contains(next)) {
								PageableResourceDescriptorImpl resDesc = resourceTable.acquire(next.getSourceResourceURI()); // FIXME
								// is
								// identical
