...
public class IndexUpdaterImpl implements IndexUpdaterInternal {
	...
	public IndexUpdaterImpl(GlobalTables _globalTables) {
		...
		this.deleteResourceCache = new LinkedHashSet<String>(); // TODO lazy initialization?
	}

	@Override
	public void deleteResource(String uri) {
		this.deleteResourceCache.add(uri); // FIXME remove from insertResourceCache
	}
	...
	public void insertEReference(String sourceResourceUri, String sourceFragment, String typeUri, String targetResourceUri,
		...
		if (desc != null) {
			...
			if (eod != null) {
				ReferenceDescriptorImpl refDesc = this.descriptorFactory.createReferenceDescriptor(eod, typeUri, targetResourceUri,
						targetFragment);
	...
	private void removeResourceDescriptor(String resource) {
		...
		if (resourceDescriptor != null && resourceDescriptor.isIndexed()) {
	...
	private void collectOutgoingLinks(PageableResourceDescriptorImpl resDesc) {
		for (Iterator<ReferenceDescriptorImpl> it = resDesc.getReferences(); it.hasNext();) {
			...
			PageableResourceDescriptorImpl targetResDesc = this.globalTables.resourceIndex.acquire(refDesc.getTargetResourceURI()); // does not need equal key access, may be changed to identical key access
	...
	private void addResourceDescriptor(ResourceDescriptor _resDesc) {
		...
		if (existingResDesc != null) {
			if (!existingResDesc.isIndexed()) {
				...
				// FIXME unnecessary remove -> identity copy in other direction, adjust paging? user would have other object on hand than indexed
