...
import de.hpi.sam.bp2009.benchframework.util.BenchframeworkResourceImpl;
...
public class ListPage extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		tbl.setWizard(((TestframeworkWizard) getWizard()));
	...
	private void generateEClassMappingAndSelectionBoxForResultProcessors(
		...
		for (ResultProcessor p : ((TestframeworkWizard) getWizard())
				.getIntImpl().getEngine().getRegisteredResultProcessors()) {
		...
		processorBox.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				ResultProcessor obj = nameToProc.get(processorBox
						.getItem(processorBox.getSelectionIndex()));
	...
	private void generateEClassMappingAndSelectionBoxForOperators(
		...
		for (Operator op : ((TestframeworkWizard) getWizard()).getIntImpl()
	...
	/**
	 * Generates an RemoveButton and add it to the Canvas this method access the
	 * operatortable tbl
	 * 
	 * @param parent
	 *            to draw on
	...
	private void buildRemoveButton(Composite parent) {
		...
		btn.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				wiz.getRun().getOperators().removeAll(
						tbl.getSelectedOperatorList());
	...
	/**
	 * Generates an LoadButton for Testruns
	 * 
	 * @param parent
	 *            to draw on
	 */
	private void buildLoadButton(final Composite parent) {
		Button btn = new Button(parent, SWT.CENTER);
		btn.setText("Load Testrun");
		btn.addSelectionListener(new SelectionListener() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				try {
					...
					if (absoluteFilePath == null)
						return;
					TestframeworkWizard wiz = ((TestframeworkWizard) getWizard());
					TestRun newRun=loadTestRun(absoluteFilePath);
					if(newRun !=null){
						wiz.setRun(newRun);
						};
					setPageComplete(true);
	...
	/**
	 * Load the first TestRun found in a resource identified by the given URI
	 * 
	 * @param absoluteFilePath
	 *            , the URI used to create a {@link BenchframeworkResourceImpl},
	 *            may cause in loading error if not well formed
	 * @return the Testrun found or null if no TestRun found or loading failed
	 * @throws IOException
	 */
	private TestRun loadTestRun(String absoluteFilePath) {
		Resource r = new BenchframeworkResourceFactoryImpl().createResource(URI
				.createFileURI(absoluteFilePath));
		try {
			r.load(null);
		} catch (IOException e) {
			e.printStackTrace();
		...
		for (EObject o : r.getContents())
			if (o.eClass().getClassifierID() == (BenchframeworkPackage.TEST_RUN))
				return (TestRun) o;
		return null;
	}

	/**
	 * Generates an AddButton and add it to the Canvas this method access the
	 * selectionbox and the nameToClass map
	 * 
	 * @param parent
	 *            to draw on
	 */
		...
		btn.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (operatorBox.getSelectionIndex() == -1)
					return;
				String name = operatorBox.getItem(operatorBox
						.getSelectionIndex());
				EClass cls = nameToEclass.get(name);

				EObject obj = cls.getEPackage().getEFactoryInstance().create(
						cls);
	...
	public boolean canFlipToNextPage() {
		return !((TestframeworkWizard) getWizard()).getRun().getOperators()
				.isEmpty();
	}

	private void addOperatorToWizard(Operator op, TestframeworkWizard wiz) {
		/*
		...
		if (op.getOption() != null && op.getOption().getWizardPage() != null) {
			...
			/*
			 * FIXME if multiple instances running some concurrency issues might
			 * result here
			 */
