...
public interface FunctionSignatureImplementation extends SignatureImplementation
{
   ...
   *        annotation="http://de.hpi.sam.bp2009.OCL body='if self.oclIsKindOf(Block) then    self.oclAsType(Block).localIsSideEffectFree()  else    if self.oclIsKindOf(CellSet) then      self.oclAsType(CellSet).localIsSideEffectFree()    else      false    endif  endif'"
