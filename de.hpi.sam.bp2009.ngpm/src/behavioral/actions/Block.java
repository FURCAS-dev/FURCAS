...
public interface Block extends FunctionSignatureImplementation, InScope
{
   ...
   *        annotation="http://de.hpi.sam.bp2009.OCL body='if self.owningStatement->notEmpty() then    self.owningStatement.block.getOutermostBlock()  else    self  endif'"
   ...
   * <!-- end-model-doc -->
   * @model kind="operation"
   *        annotation="http://de.hpi.sam.bp2009.OCL body='self.addNamedValuesWithNewNames(  -- Handle Foreach  let s:Set(NamedValue)=Set{} in  s->union(if owningStatement.oclIsKindOf(Foreach) then    owningStatement.oclAsType(Foreach).forVariable->asSet()  else    Set{}  endif)->union(  -- add parameters for those blocks that are used as a signature implementation  functionSignature.input->asSet()  )->union(  implements_.input->asSet()  ),  -- then ascend the block composition hierarchy and add all NamedValues defined in parent blocks before the occurrence of the statement with the nested block  if owningStatement->notEmpty() then    owningStatement.getNamedValuesInScope()  else    -- add formal object parameters from owning class    let oc = self.getOwningClass() in    if oc->notEmpty() then      oc.formalObjectParameters->asSet()    else      Set{}    endif  endif)'"
   *        annotation="http://www.eclipse.org/emf/2002/Ecore constraints='body'"
   * @generated
   */
   ...
   * <!-- end-user-doc -->
   * @model kind="operation"
   *        annotation="http://de.hpi.sam.bp2009.OCL body='let outermost = self.getOutermostBlock() in  let implementedSignature = outermost.getImplementedSignature() in  if implementedSignature->notEmpty() then    implementedSignature.getOwningClass()  else    null  endif'"
   *        annotation="http://www.eclipse.org/emf/2002/Ecore constraints='body'"
   * @generated
   */
