...
import com.sap.tc.moin.repository.test.jmitck.modelsem.JmiModelSemanticsScript;
 ...
 * verifies that the vendor implementations conform to the behavior specified
 * for the methods of the com.sap.tc.moin.repository.mmi.reflect package For example it is tested -
 * whether a link is created correctly - whether attribute values can be set and
 * retrieved - whether methods like refMofId, refMetaObject or refDelete behave
 * correctly There are 172 semantics tests in the JMI TCK
 */

@RunWith( MoinParameterizedRunner.class )
public class TestJmiTckModelsem {

    // name of property file containing the path to the testdescriptions
    static final String PROP_FILE_MODELSEM = "modelsem.properties"; //$NON-NLS-1$
    ...
    public void tearDown( ) throws Exception {
        ...
        // // resets MOIN
        // MoinTestHelper.getInstance().reset();
    }
     ...
     */
    @Parameters
    public static List<Object[]> returnTestParameters( ) throws FileNotFoundException, IOException, Fault {

        TestJmiTckUtil util = new TestJmiTckUtil( );
        try {
            Properties tdProps = util.loadProperties( PROP_FILE_MODELSEM );
    ...
    public static List<String> returnTestParametersDescriptions( ) throws FileNotFoundException, IOException, Fault {
        ...
        try {
            TestJmiTckUtil util = new TestJmiTckUtil( );
            tdArrayList = util.findTestDescriptions( util.loadProperties( PROP_FILE_MODELSEM ) );
        } catch ( Fault e ) {
            e.printStackTrace( );
            return null;
        }
     ...
     * @param goodResult
     */
    public TestJmiTckModelsem( TestDescription td, boolean goodResult ) {
     ...
     */
    @Test
    public void testModelSemantics( ) throws Fault {

        // instantiates a script, initializes and runs it
        Script script = new JmiModelSemanticsScript( );
