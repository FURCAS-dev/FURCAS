...
public class AddObjectMutation extends AbstractMutation {
    ...
    public boolean mutate(IModelProvider modelProvider, IMutationTracker tracker) {
        ...
        if (mutationCount % 1000 == 0) {
            ...
            System.out.println("" + sdf.format(Calendar.getInstance().getTime()) + ": Addmutations: " + mutationCount);
        }
        ...
        if (eClass != null) {
            if (modelProvider.getModelSize() == 0) {
                // we have an empty model, just instantiate a random class
                EObject o = eClass.getEPackage().getEFactoryInstance().create(eClass);
                modelProvider.getModelResource().getContents().add(o);
                return true;
            } else {
                EStructuralFeature feature = modelProvider.getRandomFeatureHavingType(eClass, false);
                if (feature != null) {
                    EObject parent = modelProvider.getRandomEObjectHavingFeature(feature);
                    if (parent != null) {
                        ...
                        try {
                            ...
                            if (success) {
                                String message = "Added " + eObject.toString() + " to " + parent.toString() + " using feature "
                                        + feature;
                                log(IStatus.INFO, message);
                                // track mutation
                                List<EObject> involvedEObjects = toEObjectList(eObject);
                                involvedEObjects.add(parent);
                                tracker.track(this.getId(), message, true, involvedEObjects, toFeatureList(feature));
                                return true;
                            }
    ...
    private EObject createInstance(EClass eClass, IModelProvider modelProvider) {
        ...
        for (EStructuralFeature feature : eClass.getEAllStructuralFeatures()) {
            if (feature.isChangeable()) {
                try {
                    instance.eSet(feature, modelProvider.getRandomValue(feature));
                } catch (ClassCastException cce) {
                    continue;
                } catch (IllegalArgumentException iae) {
                    continue;
