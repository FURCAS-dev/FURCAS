...
public class TestQueryBugs extends QueryTestCase{
	...
	/**
	 * This tests test the DirtyResourceFactory which is an eContentAdapter which keeps track of the
	 * dirty resources.
	 * This check the backward navigation which happens from divison to transport.
	 * Check https://bugs.eclipse.org/bugs/show_bug.cgi?id=324176
	 */
	@Test
	public void testDirtyResourceFactory_Bug324176(){
		ResultSet resultSet=null;
		
		String query = "select div.name, dep.name from [" 
		+ EcoreUtil.getURI(CompanyPackage.Literals.DEPARTMENT) + "] as dep, "+"["
		+ EcoreUtil.getURI(CompanyPackage.Literals.TRANSPORT) + "] as trans, ["
		+ EcoreUtil.getURI(CompanyPackage.Literals.DIVISION) + "] as div " 
		+ " where div.department = dep"
		+ " where div.commuting = trans"
		+ " where dep.budget > 60";
		...
		assertEquals(resultSet.getAttribute(0, "div", "name"), this.div2inRes2.getName());
		
	}
	
	
	/**
	 * This tests when a null resource set is passed.
