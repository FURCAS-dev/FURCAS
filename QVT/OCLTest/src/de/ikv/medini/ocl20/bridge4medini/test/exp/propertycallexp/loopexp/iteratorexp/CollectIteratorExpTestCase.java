...
public class CollectIteratorExpTestCase extends OclInterpreterTestCase {

	// TODO TODOMWA test ->flatten()

	public void testCollectIteratorWithoutType() throws Exception {
		List result = this.adapter.evaluate("Set { 1 , 2 , 3}->collect( x | x * 2 ) ");
		...
		result = this.adapter.evaluate("Set { }->collect( x | x * 2) ");
		Assert.assertEquals(result.size(), 1);
		result1 = (Collection) result.get(0);
		Assert.assertEquals(0, result1.size());
	}

	public void testCollectIteratorWithType() throws Exception {
		List result = this.adapter.evaluate("Set { 1 , 2 , 3}->collect( x : Integer | x * 2 ) ");
		Assert.assertEquals(result.size(), 1);
		Collection result1 = (Collection) result.get(0);
		Vector resultExpected = new Vector();
		resultExpected.add(this.adapter.getNumber(2));
		...
		result = this.adapter.evaluate("Set { }->collect( x : Integer | x * 2 ) ");
		Assert.assertEquals(result.size(), 1);
		result1 = (Collection) result.get(0);
		Assert.assertEquals(0, result1.size());
	}
