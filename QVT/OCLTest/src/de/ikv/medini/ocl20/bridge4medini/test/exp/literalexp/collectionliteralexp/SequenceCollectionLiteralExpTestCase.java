...
public class SequenceCollectionLiteralExpTestCase extends CollectionLiteralExpTestCase {

	public void testSequenceCollectionLiteralExp() throws Exception {
		List result = this.adapter.evaluate("Sequence { 1 , 3 , 2 , 3 }");
		...
		result = this.adapter.evaluate("Sequence { 1 .. 2 , 3 }");
		Assert.assertEquals(result.size(), 1);
		...
		result = this.adapter.evaluate("Sequence {}");
		Assert.assertEquals(result.size(), 1);
		result1 = (Collection) result.get(0);
		resultExpected = new Vector();
