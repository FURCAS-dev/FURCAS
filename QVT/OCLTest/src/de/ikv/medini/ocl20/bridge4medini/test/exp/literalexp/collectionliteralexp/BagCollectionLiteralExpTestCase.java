...
public class BagCollectionLiteralExpTestCase extends CollectionLiteralExpTestCase {

	public void testBagCollectionLiteralExp() throws Exception {
		List result = this.adapter.evaluate("Bag { 1 , 2 , 3 , 3 }");
		...
		// TODO TODOMWA test exact !!

		result = this.adapter.evaluate("Bag { 1..3 , 3 }");
		Assert.assertEquals(result.size(), 1);
		...
		result = this.adapter.evaluate("Bag {}");
		Assert.assertEquals(result.size(), 1);
		result1 = (Collection) result.get(0);
		resultExpected = new Vector();
