...
public class EReferenceQueryImpl<RDType> implements EReferenceQuery<RDType>, QueryInternal<RDType, EReferenceDescriptor> {
	...
	public Iterable<? extends EReferenceDescriptor> queryEReferenceDescriptor(final PageableResourceDescriptorImpl resDesc, final GlobalTables globalTables) {
		...
		case BACKWARD:
			if (tgtFragment == null || tgtFragment.indexOf('*') > -1) {
						...
						while (scopeIterator != null && scopeIterator.hasNext()) {
							...
							if ((getSourceScope() == null || getSourceScope().contains(next.getSourceResourceURI())) && //
								...
								if (next.isIntraLink()) {
									...
									PageableResourceDescriptorImpl resDesc = globalTables.resourceIndex.acquire(next.getSourceResourceURI()); // FIXME
									// is
									// identical
									// key
									Iterable<ReferenceDescriptorImpl> candidates = resDesc.outgoingLinkTable.getAllWithEqualKey(next.getSourceFragment());
						...
						if (e.getTargetResourceURI() == resDesc.getURI()) {
							if (e.getTargetFragment().equals(next.getTargetFragment())) {
								if (getType() == null || getType() == e.getEReferenceURI()) {
									// if (srcFragment == null ||
									// QueryUtil.matchesGlobbing(e.getSource().getFragment(),
									// srcFragment)) {
									return true;
									// }
								}
				...
				final Iterator<EReferenceDescriptorInternal> scopeIterator = resDesc.incomingLinkTable.getAllWithEqualKey(tgtFragment).iterator();
