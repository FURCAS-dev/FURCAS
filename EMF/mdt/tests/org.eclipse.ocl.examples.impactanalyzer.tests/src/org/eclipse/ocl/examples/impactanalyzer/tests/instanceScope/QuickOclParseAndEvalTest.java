...
public class QuickOclParseAndEvalTest extends TestCase {
  ...
  /**
    ...
    /**
     * Check what happens when last value of a range is invalid instead of an Integer. Interestingly, this aborts the
     * whole evaluation and even "bypasses" a trailing oclIsInvalid().
     */
    @Test
    public void testParseAndEvaluateOclExpressionWithInvalidAsLastPartOfRange() throws ParserException {
        ...
        OCLExpression expression4 = oclHelper
                .createQuery("(Sequence{1..(self.parentDepartment.subDepartment->size())}->select(i | i>0)).oclIsInvalid()");
        Object result4 = ocl.evaluate(CompanyFactory.eINSTANCE.createDepartment(), expression4);
        assertTrue(result4 instanceof DynamicEObjectImpl);
        assertEquals("OclInvalid_Class", ((DynamicEObjectImpl) result4).eClass().getName());
    }
     ...
     */
    @Test
    public void testParseAndEvaluateOclExpressionWithMixOfPrimitiveAndObjectTypesInCollectionLiteral()
    ...
    /**
     * Testing if a let-expression evaluates to OclInvalid if the initExpression evaluates to OclInvalid although the
     * variable is not used in the "in" expression.
     */
    @Test
    public void testParseAndEvaluateOclExpressionWithInvalidLetInitExpression() throws ParserException {
        OCLExpression expression4 = oclHelper.createQuery("let x:Integer=self.name.size() in 1+2");
        Object result4 = ocl.evaluate(null, expression4);
    ...
    public void testParseAndEvaluateOclExpressionForAllWithTwoIterators() throws ParserException {
        OCLExpression expression4 = oclHelper.createQuery("Set{1, 2, 3}->forAll(i, j | i+j <> 7)");
    ...
    /**
     * Testing if an iterate's accumulator expression is really evaluated even when the iterate is applied to an empty
     * collection
     */
    @Test
    ...
    /**
     * Testing if shadowing a variable by an iterator leads to incorrect results because the shadowed variable is
     * overwritten by the iterator values
     */
    @Test
    public void testParseAndEvaluateOclExpressionIteratorShadowingLetVariable() throws ParserException {
        ...
        try {
            OCLExpression expression4 = oclHelper
                    .createQuery("let i:Integer=1 in if self.ownedTypeDefinition->select(i|i.oclAsType(ClassTypeDefinition).clazz->notEmpty())->isEmpty() then i else 0 endif");
            Object result4 = ocl.evaluate(param, expression4);
            assertEquals(1, result4);
   ...
   * Ensures that an OclInvalid value propagates from a collection literal to the overall expression
    ...
    public void testParseAndEvaluateOclExpression() throws ParserException {
        OCLExpression expression = oclHelper
                .createQuery("self.ownedTypeDefinition.oclAsType(data::classes::ClassTypeDefinition).clazz");
    ...
    public void testParseAndEvaluateOclExpressionWithPropertyCallOnNullValue() throws ParserException {
        param.setOwnedTypeDefinition(null);
        OCLExpression expression2 = oclHelper
                .createQuery("self.ownedTypeDefinition.oclAsType(data::classes::ClassTypeDefinition).clazz.oclIsInvalid()");
    ...
    public void testParseAndEvaluateOclExpressionWithCollectOverNullValue() throws ParserException {
        param.setOwnedTypeDefinition(null);
        OCLExpression expression3 = oclHelper
                .createQuery("self.ownedTypeDefinition.oclAsType(data::classes::ClassTypeDefinition)->collect(clazz)");
    ...
    public void testParseAndEvaluateOclExpressionWithImplicitCollectOverOperationCallResult() throws ParserException {
        OCLExpression expression4 = oclHelper
                .createQuery("self.ownedTypeDefinition.oclAsType(data::classes::ClassTypeDefinition).clazz.getAssociationEnds().otherEnd()");
        Object result4 = ocl.evaluate(param, expression4);
    ...
    public void testParseAndEvaluateOclExpressionWithDelegatesToSimulation() throws ParserException {
        OCLExpression expression4 = oclHelper
                .createQuery("self.ownedTypeDefinition.oclAsType(data::classes::ClassTypeDefinition).clazz.getAssociationEnds().otherEnd()->select(delegation->notEmpty()).type.clazz->reject(c|c=self)->asSet()");
        Object result4 = ocl.evaluate(param, expression4);
        assertTrue(((Collection<?>) result4).isEmpty());
    }
