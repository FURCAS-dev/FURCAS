package com.sap.river.interpreter.test;
...
public class TestBasicExpressions extends TestCase {
	...
	try {
	    ...
	    SapClass stringClass = MetamodelUtils.findClass(conn, "String");
    	    ...
    	    NativeObject result = (NativeObject) new RiverInterpreter(conn, new InMemoryRepository()).evaluate(nl);
	...
	try {
	    ...
	    assertEquals(new Fraction(12345),
		    ((NativeObject) new RiverInterpreter(conn, new InMemoryRepository()).evaluate(mce)).getNativeObject());
	} catch (Exception e) {
	    throw new RuntimeException(e);
	}
    }
	...
	try {
	    ...
	    RiverInterpreter interpreter = new RiverInterpreter(conn, new InMemoryRepository());
	    RiverObject result = interpreter.evaluate(fce);
	...
	try {
	    ...
	    Iterator<RiverObject> resultIterator = result.flatten().iterator();
	...
	try {
	    ...
	    RiverInterpreter interpreter = new RiverInterpreter(conn, new InMemoryRepository());
	    RiverObject result = interpreter.evaluate(mce);
	    
	    Iterator<RiverObject> resultIterator = result.iterator();
	    assertEquals(new Fraction(-42), ((NativeObject) resultIterator.next()).getNativeObject());
	    assertEquals(new Fraction(-43), ((NativeObject) resultIterator.next()).getNativeObject());
	} catch (Exception e) {
	    throw new RuntimeException(e);
	...
	try {
	    ...
	    */
	    ...
	    assertEquals(new Fraction(12345+12345+45678),
		    ((NativeObject) new RiverInterpreter(conn, new InMemoryRepository()).evaluate(fce)).getNativeObject());
	} catch (Exception e) {
	    throw new RuntimeException(e);
	}
    }
	...
	try {
	    ...
	    impl.getStatements().add(ret);
	    FunctionCallExpression fce = MetamodelUtils.createFunctionCallExpression(conn, sig);
	    RiverObject evalResult = new RiverInterpreter(conn, new InMemoryRepository()).evaluate(fce); // multi-valued because
	    // SalesOrder-->SalesOrderItem is multi-valued
	    Iterator<RiverObject> iter = evalResult.flatten().iterator();
	...
	try {
	    ...
	    FunctionCallExpression fce = MetamodelUtils.createFunctionCallExpression(conn, sig);
	    RiverObject evalResult = new RiverInterpreter(conn, new InMemoryRepository()).evaluate(fce);
