...
import com.sap.river.interpreter.DebugSession;
import com.sap.river.interpreter.RiverInterpreter;

public class RunletDebugTarget extends DebugElement implements IDebugTarget, DebugListener {
    private Main main;
    private RiverInterpreter topLevelInterpreter;
    ...
    public RunletDebugTarget(Main main, ILaunch launch, String name) {
    ...
    private Collection<RunletThread> getThreadsForInterpreter(RiverInterpreter interpreter) {
	Collection<RunletThread> threads = new LinkedList<RunletThread>();
	threads.add(new RunletThread(interpreter, getLaunch(), this));
	for (RiverInterpreter child : interpreter.getRunningChildren()) {
    ...
    @Override
    public void started(RiverInterpreter interpreter) {
    ...
    @Override
    public void hitBreakpoint(RiverInterpreter interpreter, RefObject element) {
	for (RunletThread thread : getThreadsForInterpreter(interpreter)) {
    ...
    @Override
    public void resumed(RiverInterpreter interpreter, ResumeReason reason) {
	for (RunletThread thread : getThreadsForInterpreter(interpreter)) {
	    int detail;
	    switch (reason) {
	    case STEP_INTO:
    ...
    @Override
    public void suspended(RiverInterpreter interpreter, SuspendReason reason) {
	for (RunletThread thread : getThreadsForInterpreter(interpreter)) {
	    int detail;
	    switch (reason) {
	    case BREAKPOINT:
    ...
    @Override
    public void terminated(RiverInterpreter interpreter) {
	for (RunletThread thread : getThreadsForInterpreter(interpreter)) {
