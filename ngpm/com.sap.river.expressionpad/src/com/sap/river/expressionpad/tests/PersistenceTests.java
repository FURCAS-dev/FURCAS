...
public class PersistenceTests extends RiverTestCase {
    ...
    public void testCommitAcrossSessions() throws Exception {
        main.execute(
        	...
        	"o.persons += p",
        	"commit");
        Repository repository = main.getRepository();
        Main main2 = new Main(main.getConnection(), repository);
        RiverObject[] result = main2.execute(
    ...
    public void testDeleteOfPersistentInstanceWithInBetweenCommit() throws Exception {
        main.execute(
        	...
        	"store p1",
        	"var s1 = commit");
        ExecuteResult result = main.execute(
    		"delete p1",
    		"commit",
    		"p1.name == \"Jan\"",
	...
	main.execute("var o = new Organization", 
		...
		"o.persons += p", 
		"var s1 = commit");
	RiverObject[] result2 = main.execute("all[s1.when()] Organization.persons.count")
		.getResult();
	...
	main.execute(
		...
		"c.name = \"Berlin\"", 
		"commit");
	RiverObject[] result = main.execute("all[all] City.name").getResult();
	// one Multivalue object
	assertEquals(1, result.length);
	...
	main.execute(
		...
		"store c", 
		"commit");
	RiverObject[] result = main.execute("all[changed] City.name").getResult();
	// one Multivalue object
	assertEquals(1, result.length);
	assertTrue(result[0] instanceof MultiValuedObject);
	MultiValuedObject mvo = (MultiValuedObject) result[0];
	...
	main.execute(
		...
		"o.persons += new Person",
		"commit");
	RiverObject[] result = main.execute("all[changed] Organization").getResult();
	// one Multivalue object
	assertEquals(1, result.length);
	assertTrue(result[0] instanceof MultiValuedObject);
	MultiValuedObject mvo = (MultiValuedObject) result[0];
	...
	assertEquals(4, result.getResult().length);
    }
	...
	Repository rep = new InMemoryRepository();
	Main main1 = new Main("ngpm.stdlib", rep);
        ExecuteResult executeResult1 = main1.execute(
            "var c1=new City",
            "var p=new Person",
            "c1.name=\"Mannheim\"",
            ...
            "store p",
            "var s1=commit");
        RiverObject[] result1 = executeResult1.getResult();
        assertEquals(6, result1.length);
        Main main2 = new Main("ngpm.stdlib", rep);
        ExecuteResult executeResult2 = main2.execute(
        	"var p=all Person->iterate(p|p)",
                "var c2=new City",
                "c2.name=\"Heidelberg\"",
                "p.addresses = p.addresses->replace(city=c2)",
        	"p.addresses.city.name");
        RiverObject[] result2 = executeResult2.getResult();
	...
	Main main1 = new Main("ngpm.stdlib", rep);
        ...
        Main main3 = new Main("ngpm.stdlib", rep);
        ExecuteResult executeResult3 = main3.execute(
                "Quotation q=all Quotation->iterate(q|q)",
                "q.responsibleEmployee.name");
        RiverObject[] result3 = executeResult3.getResult();
	...
	Repository r = main.getInterpreter().getRepository();
	SnapshotIdentifier base = main.getInterpreter().getDefaultSnapshot();
	ChangeSet cs1 = new ChangeSetImpl();
	Snapshot ss1 = r.apply(cs1, base);
	ChangeSet cs2 = new ChangeSetImpl();
	...
	ChangeSet cs3 = new ChangeSetImpl();
    ...
    public void testAddingLinkToExistingButNotYetLoadedLinkCollection() throws Exception {
        ...
        Repository repository = main.getRepository();
        Main main2 = new Main(main.getConnection(), repository);
        RiverObject[] result2 = main2.execute(
        ...
        Main main3 = new Main(main.getConnection(), repository);
        RiverObject[] result3 = main3.execute(
            	"var o = all Organization",
            	"o.persons.name.count",
