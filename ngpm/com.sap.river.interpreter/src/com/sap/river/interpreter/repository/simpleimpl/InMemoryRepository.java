...
import com.sap.river.interpreter.operationaltransformation.ChangeList;
import com.sap.river.interpreter.operationaltransformation.EmptyChange;
...
public class InMemoryRepository implements Repository {
    private Map<Snapshot, SimpleLinkContainerImpl> linkContainers;
    private Map<Snapshot, Set<EntityObject>> entities;
    private Map<Snapshot, Map<SapClass, Set<EntityObject>>> entitiesByClass;
     ...
     */
    protected Map<Pair<Snapshot, Snapshot>, ChangeSet> changes;
	...
	linkContainers = new HashMap<Snapshot, SimpleLinkContainerImpl>();
	entities = new HashMap<Snapshot, Set<EntityObject>>();
	entitiesByClass = new HashMap<Snapshot, Map<SapClass, Set<EntityObject>>>();
	tipsOfBranches = new HashSet<Snapshot>();
	predecessors = new HashMap<Snapshot, Set<Snapshot>>();
	changes = new HashMap<Pair<Snapshot, Snapshot>, ChangeSet>();
	...
	addSnapshot(result, /* predecessor */ null, null,
		new SimpleLinkContainerImpl(), new HashSet<EntityObject>(), new HashMap<SapClass, Set<EntityObject>>());
	return result;
    }

    @Override
    public Collection<Link> all(Association association, SnapshotIdentifier fromSnapshot) {
	/** check invariants */
    ...
    @Override
    public Collection<EntityObject> all(SapClass clazz, SnapshotIdentifier fromSnapshot) {
	Collection<EntityObject> result;
	/** check invariants */
	...
	resolveIfNotBound(fromSnapshot);
	assert has(fromSnapshot.getSnapshot());
	Map<SapClass, Set<EntityObject>> entitiesByClassFromSnapshot = getEntitiesByClass().get(fromSnapshot.getSnapshot());
	if (entitiesByClassFromSnapshot != null) {
	    Set<EntityObject> allEntitiesFromSnapshot = entitiesByClassFromSnapshot.get(clazz);
    ...
    @Override
    public Snapshot apply(ChangeSet changeset, SnapshotIdentifier applyTo) {
	/** check invariants */
	...
	Snapshot result = new SnapshotImpl();
	SimpleLinkContainerImpl snapshotLinkContainer = getLinkContainers().get(applyTo.getSnapshot()).clone();
	Set<EntityObject> snapshotEntities = new HashSet<EntityObject>();
	for (EntityObject eo : getEntities().get(applyTo.getSnapshot())) {
	    snapshotEntities.add(eo.clone());
	}

	for (Iterator<RepositoryChange> i = changeset.getChanges(); i.hasNext(); ) {
	    RepositoryChange change = i.next();
	    change.apply(snapshotEntities, snapshotLinkContainer);
	}
	Map<SapClass, Set<EntityObject>> snapshotEntitiesByClass = new HashMap<SapClass, Set<EntityObject>>();
	for (EntityObject eo:snapshotEntities) {
	    Set<EntityObject> set = snapshotEntitiesByClass.get(eo.getType().getClazz());
	    if (set == null) {
		set = new HashSet<EntityObject>();
		snapshotEntitiesByClass.put(eo.getType().getClazz(), set);
	    }
	    set.add(eo);
	}
    ...
    private void addSnapshot(Snapshot snapshot, SnapshotIdentifier predecessor,
	    ChangeSet changesFromPredecessor, SimpleLinkContainerImpl snapshotLinkContainer,
	    Set<EntityObject> snapshotEntities, Map<SapClass, Set<EntityObject>> snapshotEntitiesByClass) {
    ...
    private Map<Snapshot, SimpleLinkContainerImpl> getLinkContainers() {
        return linkContainers;
    }

    private Map<Snapshot, Set<EntityObject>> getEntities() {
        return entities;
    }

    private Map<Snapshot, Map<SapClass, Set<EntityObject>>> getEntitiesByClass() {
    ...
    @Override
    public Collection<Link> loadLinks(ClassTypedObject from, AssociationEnd otherEnd) {
	assert (from instanceof ValueObject) || from.isPersistent();
	SnapshotIdentifier si = from.getOrigin();
	resolveIfNotBound(si);
	assert has(si.getSnapshot());
	SimpleLinkContainerImpl lc = getLinkContainers().get(si.getSnapshot());
	...
	Change changesOnBranch = getChanges(pathIter.next());
	Change changesOnTrunk = getChanges(pathIter.next());
	Transformer t = new Transformer();
	Pair<Change, Change> mergingChanges = t.transform(changesOnBranch, changesOnTrunk);
	ChangeSet mergingChangeSetForTrunk = new ChangeSetImpl(mergingChanges.getB());
	...
	changes.put(new Pair<Snapshot, Snapshot>(fromBranch, mergeResult), new ChangeSetImpl(mergingChanges.getA()));
	return mergeResult;
    }

    protected Change getChanges(List<Snapshot> snapshotList) {
	Change result;
	...
	if (!iter.hasNext()) {
		...
		while (iter.hasNext()) {
		    ...
		    successor = iter.next();
		    ChangeSet changeSetBetweenPredAndSucc = changes.get(new Pair<Snapshot, Snapshot>(predecessor, successor));
	...
	private EntityObject delegate;
	public EntityObjectWrapper(EntityObject o) {
	    delegate = o;
	}
	public boolean equals(Object o) {
	    if (o instanceof EntityObject) {
		return delegate.logicallyEquals((RiverObject) o);
	    } else if (o instanceof EntityObjectWrapper) {
	...
	public boolean equals(Object o) {
	    ...
	    } else if (o instanceof LinkWrapper) {
