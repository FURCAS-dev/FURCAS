...
/**
 * This is an ANTLR specific implementation of a token stream that is used to not only pass the antlr tokens to the parser but
 * also tracks the corresponding model elements and {@link AbstractToken}s hat were produced during lexing.
 * 
 * @author C5106462
 */
public class ANTLRIncrementalTokenStream extends CommonTokenStream implements ITextBlocksTokenStream {
    ...
    /**
     * @see CommonTokenStream#skipOffTokenChannels, also prepares the model element token for consumption.
     */
    @Override
    public void consume() {
        if (p < tokens.size()) {
    ...
    /**
     * @see CommonTokenStream#skipOffTokenChannels , also skips the corresponding model element tokens.
     */
    @Override
    ...
    /**
     * Adds all tokens from the given <code>startIndex</code> to <code>nextOnChannelTokenIndex</code> to the
     * {@link #offChannelTokens} and removes them from the tokenToModelElement list of the lexer.
     * 
     * @param startIndex
     * @param nextOnChannelTokenIndex
     */
    private void skipOffChannelsModelElementTokens(int startIndex, int nextOnChannelTokenIndex) {
    ...
    public AbstractToken consumeTokenModelElementForParserToken(Token parserToken) {
        ...
        if (result == null) {
            throw new RuntimeException("No ModelElement Token found for:" + parserToken);
        }
        return result;
    }
