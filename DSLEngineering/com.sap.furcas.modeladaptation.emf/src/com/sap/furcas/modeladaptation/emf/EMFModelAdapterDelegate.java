...
/**
 * Functionality Delegate for MOINModelAdapter. This class contains the core
 * AdapterFunctionality, while it delegates repeated and tedious JMI tasks to a
 * helper class.
 */
public class EMFModelAdapterDelegate {
    ...
    /**
     * <tt>value</tt> can be a collection of objects which then will all be set
     * at once for a multi-valued feature <tt>propertyName</tt>
     */
	...
	if (oclQuery.startsWith(OCL_QUERY_PREFIX)) {
	    refObject = jmiHelperDelegate.findElementWithOCLQuery(sourceModelElement, referencePropertyName, keyValue,
		    oclQuery.substring(OCL_QUERY_PREFIX.length()), contextObject, foreachObject);
	} else {
	    refObject = jmiHelperDelegate.findElementWithOCLQuery(sourceModelElement, referencePropertyName, keyValue, oclQuery,
		    contextObject, foreachObject);
	}
	...
	if (oclQuery.startsWith(OCL_QUERY_PREFIX)) {
	    refObject = jmiHelperDelegate.findElementsWithOCLQuery(sourceModelElement, referencePropertyName, keyValue,
