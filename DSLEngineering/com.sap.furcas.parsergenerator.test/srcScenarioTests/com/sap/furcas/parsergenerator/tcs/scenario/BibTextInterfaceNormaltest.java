...
public class BibTextInterfaceNormaltest extends GeneratedParserBasedTest{

	private static final String LANGUAGE = "BibTextInterfaceNormalTest";
	private static final List<File> LANGUAGE2 = new ArrayList<File>();

	private static final File TCS = ScenarioFixtureData.BIBTEXT_INTERFACE_NORMAL_TCS;
	...
	public static void setupParser() throws Exception {
		...
		for (TCSSyntaxContainerBean tcsSyntaxContainerBean : syntaxBean) {
			if (tcsSyntaxContainerBean.getSyntax().getName()
					.equalsIgnoreCase("BibtextInterfaceNormalTest")) {
		...
		Configuration configure = getConfigurationOfInterface(
				...
				syntaxBean1.getSyntax().getRequiredTemplates().iterator().next());
		...
		ParserFacade facade = generateParserForLanguageWithConfig(syntaxBeanWithConfig,
