...
public class BibTextImportTest extends GeneratedParserBasedTest {

	private static final String LANGUAGE = "BibtextImport";
	private static final List<File> LANGUAGE2 = new ArrayList<File>();

	private static final File TCS = ScenarioFixtureData.BIBTEXT_IMPORT_TCS;
	...
	public static void setupParser() throws Exception {
		...
		for (TCSSyntaxContainerBean tcsSyntaxContainerBean : syntaxBean) {
			if (tcsSyntaxContainerBean.getSyntax().getName().equalsIgnoreCase("BibtextImport")) {
				syntaxBean1 = tcsSyntaxContainerBean;
			}
		}
			ParserFacade facade = generateParserForLanguage(
					syntaxBean1, testConfig, new ClassLookupImpl());
