...
import static com.sap.mi.textual.parsing.textblocks.TbNavigationUtil.isFirstInSubTree;
import static com.sap.mi.textual.parsing.textblocks.TbUtil.getAbsoluteOffset;
import static com.sap.mi.textual.textblocks.model.TbReplacingHelper.extendLeftToOffsetAscending;
import static com.sap.mi.textual.textblocks.model.TbReplacingHelper.getGapBlanks;
	...
	public String get (final int regionOffset, final int regionLength) {
		...
		if (regionOffset > rootBlock.getLength()) {
			throw new IllegalArgumentException(
					"regionOffset larger than document length");
		}
        ...
        if (token == null) { 
            // start of region is before first token, need to fill blanks up to first token of until remaining length is full
		...
		while (remainingLength > 0) {
		    // in each step of the loop, we need to fill up the StringBuilder with
		    // token contents and blanks until remainingLength is zero.

		    	String value = token.getValue();
//		    	String resynchronizedValue = shortPrettyPrinter.resynchronizeToEditableState(token);
...
//		    		|| (token.getValue() != null && !"".equals(value) && !resynchronizedValue.equals(token.getValue()))) {
////		    	    if(token.getVersion().equals(VersionEnum.PREVIOUS)) {
////		    		if(token.getValue().equals(TbVersionUtil.getOtherVersion(token, VersionEnum.REFERENCE).getValue())) {
////		    		    replace(getAbsoluteOffset(token), token.getLength(), resynchronizedValue);
...
//			if (token.getLength() != value.length()) { // Sanity check helps to detect bugs
//			    TextBlock workingCopy = (TextBlock) TbReplacingHelper.getOrCreateWorkingCopy(getRoot());
//			    replaceInNonEmptyTree(getAbsoluteOffset(token), value.length(), value, workingCopy);
//			}
...
//			    This mean the current floor token ends before the startoffset we are interested in. We now need to 
//			    add blanks until the next token, or remaining length is filled with blanks.
			    while (remainingLength > 0) {
			        result.append(' ');
			        currentAbsoluteCursorPosition++;
			        remainingLength --;
			    ...
			    continue; 
			    // either remainingLength is zero, then the outer while will also stop
			...
			if (remainingLength > resultPartLength) {
				...
				if ( ! newToken.equals(token)) {
				    token = newToken;
				    entryOffset = getAbsoluteOffset(token);
				    startOffsetInToken = 0; // from now on startOffset is always zero, always consider full token value
				} else {
			    ...
			    result.append(value.substring(startOffsetInToken, startOffsetInToken
			            + remainingLength));
				break;
			}
		...
		if (resultString.length() != regionLength) {
			throw new IllegalStateException("Bug: Resulting String '" + resultString
					+ "' has inconsistent length " + regionLength + "!="
					+ resultString.length());
...
//		            }
     ...
     * @param workingCopy
     */
    private void replaceInNonEmptyTree(int replacedRegionAbsoluteOffset,
            int replacedRegionLength, String newText, TextBlock workingCopy) {
...
//              extend a token in the block with blanks to cover the beginning of the gap.
            TextBlock bottomBlock = (TextBlock) bottomNode;
            if (newText.length() > 0) {
                // prepare replacement by modifying the situation, prefer to extend the region over the previous token, if one exists
                ...
                if (floorToken != null) {
                    ...
                    // change replaced region text and offset / length to cover floor token
                    int distance = replacedRegionAbsoluteOffset - (floorTokenAbsoluteOffset+floorToken.getLength());
                    String gapBlanks = getGapBlanks(distance);
                    // now, replace whole floortoken up to end of replaced region
                    ...
                    /* 
                     * get ceiling token from offset, which must be somewhere below the bottom block (else there'd be a floor token)
                     * actually the first non BOS token in root must be the next token, since there was no floor token.
                     * extend ceiling token and all its parents up to bottom block to cover replaced region
                     */
                    AbstractToken ceilingToken = TbNavigationUtil.firstTokenWithoutBOS(bottomBlock);
                    extendLeftToOffsetAscending(ceilingToken, replacedRegionAbsoluteOffset);
                }
            }
        }
        // at this point it should hold that the tree is still consistent, and there exists a token at replacedRegionOffset.
     ...
     * @param currentBlock
     * @param replacedRegionRelativeOffset relative to root offset
     * @param replacedRegionLength
     * @param newText
     * @param coverageType
     * @param bottomBlock null or the block in which the gap exists
    ...
    private boolean modifyBlockRecursive(TextBlock currentBlock, int replacedRegionRelativeOffset, int replacedRegionLength,
        ...
        traverseSubTokens(currentBlock, replacedRegionRelativeOffset,
                replacedRegionLength, newText, currentBlockAbsoluteOffset);
       
        traverseSubBlocks(currentBlock,
                replacedRegionRelativeOffset, replacedRegionLength, newText,
                currentBlockAbsoluteOffset);
        ...
        if (TbNavigationUtil.getSubNodesSize(currentBlock) == 0) {
            ...
            if (coverageType.isCovered()) { // should always be the case, else this method would not be entered
                if (! coverageType.isNodeStartsLater() && ! coverageType.isNodeEndsLater()) { 
                    // textBlock starts earlier or at the same time, but ends before replacedregion end

                    // offset remains the same
                    // length may change to offsetInNode plus newText.length
                    int newBlocklength = replacedRegionRelativeOffset + newText.length(); // length change may be positive, negative or zero
                    if (newBlocklength != currentBlock.getLength()) {
                        currentBlock.setLength(newBlocklength);
                    }
                } else if (coverageType.isNodeStartsLater() && coverageType.isNodeEndsLater()) { // textblock starts within region and ends outside
                    // length is reduced by overlap of region and block, since we cut off start of block
                    int overLapSize = (replacedRegionRelativeOffset + replacedRegionLength); // always positive in this case
                    if (overLapSize != 0) {
                    ...
                    if (currentBlock.isOffsetRelative() && (replacedRegionRelativeOffset + currentBlock.getOffset()) < 0) {
                        // block relative offset moves by the region that will be cut off from its parent
                ...
                } else { // textblock is within replaced region, but has a subBlock or token left at replacedRegionStart
                    // offset remains the same
                    // length may change by the difference of replaced region and new text
                    int regionLengthDifference = newText.length() - replacedRegionLength; // length change may be positive, negative or zero
     ...
     * @return
     */
    private void traverseSubBlocks(TextBlock currentBlock,
            int replacedRegionRelativeOffset, int replacedRegionLength,
            String newText, 
        ...
        for (Iterator<TextBlock> iterator = subBlocks.iterator(); iterator.hasNext();) {
            // TODO: to optimize performance, find (binary search) and start with first child covering region
            ...
            if (coverageType.isCovered() == false) {
                if (coverageType.isNodeStartsLater() == false) { // node is before replaced region
                ...
                if (coverageType.isNodeRealInside()) {
                    ...
                    boolean needsDeleting = modifyBlockRecursive(textBlock, replacedRegionRelativeOffset-blockRelativeOffset, replacedRegionLength, newText, coverageType);
                    if ( needsDeleting) {
                        toBeDeleted.add(textBlock);
                    }
                }
     ...
     * @param currentBlockAbsoluteOffset
     */
    private void traverseSubTokens(TextBlock current,
            int replacedRegionRelativeOffset, int replacedRegionLength,
        ...
        for (Iterator<AbstractToken> iterator = subtokens.iterator(); iterator.hasNext();) { 
            // to optimize performance, find (binary search) and start with first child covering region
            ...
            if (! iterator.hasNext() && abstractToken instanceof Eostoken) {
                ...
                boolean needsDeleting = modifyTokenOnOverlap(abstractToken, currentBlockAbsoluteOffset, replacedRegionRelativeOffset, replacedRegionLength, newText, shortPrettyPrinter);
                if (needsDeleting) {
    ...
    static void replaceInEmptyTree(String newText, TextBlock workingCopy) {
        ...
        if (workingCopy.getTokens().size() == 2) {
            ...
            if (workingCopy.getTokens().size() != 3) {
                throw new IllegalArgumentException("Method only works for textBlocks of length 0 if only BOS and EOS, or BOS, an empty token, and EOS are present.");
            }
        }
    ...
    /**
     * replaces the content of a textBlock tree by modifying and deleting tokens.
     * assumes that root is passed in, and that there is a PREVIOUS version of root 
     * which contains PREVIOUS versions of all nodes. This method currently does 
     * not create any PREVIOUS versions of anything.
     ...
     * @param newText
     */
    public void replace(TextBlock root, int replacedRegionAbsoluteOffset, int replacedRegionLength,
            String newText) {
        ...
        if (root.getLength() == 0 ) {
            ...
            replaceInNonEmptyTree(replacedRegionAbsoluteOffset,
                    replacedRegionLength, newText, workingCopy);
            
        }
        workingCopy.setChildrenChanged(true);
        updateBlockCachedString(workingCopy, replacedRegionAbsoluteOffset, replacedRegionLength,
                newText);
    }
     ...
     * in the list, but the list has a minimum number of entries, given the above 
     * restrictions.
     * This implies that it is not the absolute minimal root set, because the first 
     * and last leaf nodes are always included, even if the list could become 
     * smaller if some ancestor of them were instead included.
     * 
     ...
     *         offsetTo
     */
    public List<DocumentNode> getNodesBetweenAsRootSet(
            TextBlock rootBlock, int offsetFrom, int offsetTo) {
        List<DocumentNode> results = new ArrayList<DocumentNode>();

        if (offsetFrom < 0) {
            throw new IllegalArgumentException(
                    "invalid region: offsetFrom is negative");
        }

        if (offsetTo < offsetFrom) {
            throw new IllegalArgumentException(
                    "invalid region: offsetTo is smaller than offsetFrom");
        }
	 ...
	 * offsetFrom and offsetTo, and nodes between them. The nodes between the
	 * tokens are returned as a root set, meaning that the highest level nodes
	 * are returned, such that all and only model elements between the tokens are
	 * directly or indirectly (as children, children's children etc.) contained
	 * in the list, but the list has a minimum number of entries.
	 * The list thus will never contain any 2 document nodes where one 
	 ...
	 *         offsetTo
	 */
	 private List<DocumentNode> getNodesBetweenAsRootSet(
	         DocumentNode leftFloorToken, DocumentNode rightFloorToken) {
		List<DocumentNode> results = new ArrayList<DocumentNode>();
		...
		Map<Integer, DocumentNode> rightNodeLevelMap = TbUtil.createNodeLevelMap(
		...
		DocumentNode possibleSameLevelRightNode = rightNodeLevelMap
				.get(curLeftLevel);

		// ascend
		DocumentNode parentBlock = getParentBlock(curLeftNode);
		while (parentBlock != null) {
			addRightSiblingsBeforeEndNode(results, curLeftNode,
					possibleSameLevelRightNode);
		...
		while (nextRightNode != rightFloorToken) {
			...
			if (!isFirstInSubTree(nextRightNode)) {
				DocumentNode firstSubNode = getSubNodeAt(
						(TextBlock) curRightNode, 0);
				results.add(firstSubNode);
				addRightSiblingsBeforeEndNode(results, firstSubNode,
						nextRightNode);
			}
	 ...
	 /**
		 ...
		 *            subNodes
		 */
		public void addRightSiblingsBeforeEndNode(List<DocumentNode> nodes,
				DocumentNode start, DocumentNode end) {
			if (start == end) {
				return;
			}
