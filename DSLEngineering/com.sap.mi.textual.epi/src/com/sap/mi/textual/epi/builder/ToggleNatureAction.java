...
import com.sap.mi.textual.epi.util.EclipseProjectBuildStarter;
	...
	public void run(IAction action) {
		if (selection instanceof IStructuredSelection) {
			for (Iterator<?> it = ((IStructuredSelection) selection).iterator(); it
				...
				if (project != null) {
					Shell shell = targetPart
                    .getSite()
                    .getShell();
					// Add the nature, first let the user select a Metamodel Reference configuration, then set it
		...
		try {
			...
			for (int i = 0; i < natures.length; ++i) {
				if (SyntaxGenerationNature.NATURE_ID.equals(natures[i])) {
					MessageBox questionDiag = new MessageBox(shell,
					...
					if (questionDiag.open() == SWT.YES) {
						...
						MessageBox confirmationDiag = new MessageBox(shell,
