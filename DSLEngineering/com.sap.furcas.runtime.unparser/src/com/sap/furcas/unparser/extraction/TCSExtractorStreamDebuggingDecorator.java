...
/**
 * A decorator for the {@link TCSExtractorStream} that can help in debugging the
 * pretty printer.
 */
public class TCSExtractorStreamDebuggingDecorator implements TCSExtractorStream {

    TCSExtractorStream target;

    public TCSExtractorStreamDebuggingDecorator(TCSExtractorStream target) {
	this.target = target;
    }
	...
	print("<ENTER_ALTERNATIVE>");
	target.enterAlternative(alt);
    }

    @Override
    public void beginAlternativeChoice(int choiceIndexInAlternative) {
	print("<CHOSEN_ALTERNATIVE with index: " + choiceIndexInAlternative + ">");
	target.beginAlternativeChoice(choiceIndexInAlternative);
    }

    @Override
    public void exitAlternative() {
	print("<EXIT_ALTERNATIVE>");
	target.exitAlternative();
    }
