...
public final class EcoreMetaProjectConf implements IProjectMetaRefConf {
    ...
    public ReferenceScopeBean getMetaLookUpForProject() {
        ...
        try {
            IProject[] projects = referencedProject.getReferencedProjects();
            for (int i = 0 ; i < projects.length ; i++){
                if (projects[i].getName().matches(referencedProject.getName()+".metamodel")){
                    connection = BuildHelper.getResourceSetForProject(projects[i]);
                    String metamodelPath = projects[i].getName() + "/model/mydsl.ecore";
                    URI fileURI = URI.createPlatformResourceURI(metamodelPath, true);
                    connection.createResource(fileURI);
                }
            }
        } catch (CoreException e) {
