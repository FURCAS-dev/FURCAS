...
import com.sap.furcas.textual.textblocks.testutils.FixtureWithGapsProvidingTextBlockTest;

public class TestTextBlocksModelWithGaps extends FixtureWithGapsProvidingTextBlockTest {
	...
	for (int length = 0; length <= completeFixtureText.length(); length++) {
		...
		assertEquals("region at offset " + offset + " of length " + length,
			completeFixtureText.substring(offset, offset + length), result);
	    }
	}
    }
	...
	// should change
	assertEquals("axxxxxyyyyxxxxyyyyxxxxyyyyxxxxyyyyxxxx", lexed1.getValue());
	assertEquals("ddddd", lexed4.getValue());
