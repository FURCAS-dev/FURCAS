...
public class CtsHistoryDocumentProvider extends CtsDocumentProvider {
    ...
    public CtsHistoryDocumentProvider(FurcasDocumentSetupParticpant documentSetupParticipant) {
	super(documentSetupParticipant);
    }

    @Override
    protected CtsDocument createDocumentFactoryMethod(Object element) {
	CtsDocument document = new CtsHistoryDocument(((ModelEditorInput) element));
	return document;
    }
	...
	if (document instanceof CtsHistoryDocument) {
	    ...
	    if (!ctsDocument.getRootObject().is___Alive()) {
