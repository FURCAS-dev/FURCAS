	...
	public void customizeDocumentCommand(IDocument d,DocumentCommand c) {
		...
		try {
			IConfigurationElement[] config = Platform.getExtensionRegistry()
					.getConfigurationElementsFor("com.sap.ide.cts.editor.autoEditing");
			if (config.length!=0){
				...
				if (autoEditingType.indexOf("static")!=-1){
					...
					if (autoEditingType.equals("static (configurable)")){
						String autoIndentBraces=e.getAttribute("autoIndentBraces"); 
				...
				if (autoEditingType.equals("dynamic (configurable)")){
					final Object o = e.createExecutableExtension("dynamicAutoEditingClass");
