...
public class MoinModelAdapterDelegate {
    ...
    public MoinModelAdapterDelegate(EPackage root, final ResourceSet connection, Set<URI> referenceScope, Set<URI> additionalCRIScope) {
     ...
     */
    @Deprecated
    MoinModelAdapterDelegate(AdapterJMIHelper adapterHelper) {
    ...
    public void set( EObject refAObject, String propertyName, Object value)
    throws ModelAdapterException {
        ...
        if (feat != null) {
            if (feat.getUpperBound() == 1) {
                ...
                if (feat.getUpperBound() < 0 || multiProperty.size() < feat.getUpperBound()) {
                    ...
                    throw new ModelAdapterException("Cannot add value, property " + propertyName + " has an upper multiplicity of " + feat.getUpperBound());
                }
            }       
        }
    }
    ...
    @SuppressWarnings("unchecked")
    public void set( EObject refAObject, String propertyName, Object value, int index)
    throws ModelAdapterException {
	...
	// if value is a mock, set to real object instead, may require creating the real object from the mock first
        ...
        // property may either be an attribute or a reference. First checking EAttribute, if not then check reference.
        EStructuralFeature feat = (EStructuralFeature) EcoreHelper.lookupElementExtended(mofClass, propertyName);
        if (feat != null) {
                if (feat.getUpperBound() == 1) {
                    refAObject.eSet(feat, value);
     ...
     */
    public Object setReference(EObject sourceModelElement,
...
//            contents = getContentsIncludingInherited((Namespace) contextObject);
            ...
            for (Iterator<EObject> iterator = contents.iterator(); iterator.hasNext();) {
            	EObject loopCandidateModelElement = iterator.next();
...
//      }
     ...
     * @return
     */
    private List<EObject> getContainedElements(
     ...
     * @return
     */
    private List<EModelElement> getContentsIncludingInherited(
            EModelElement contextObject) {
     ...
     * @throws ModelAdapterException 
     */
    public Object setQueriedReference(EObject sourceModelElement,
    ...
    public Collection<?> getOclEvaluation(EObject sourceModelElement,
