 ...
 * 
 */
public class TestUtil extends UtilFixtureBase {
    ...
    public void testIsAncestorTrue() throws Exception {
        ...
        block1.setParentBlock(block2);
        block2.setParentBlock(block3);
	...
	public void testCreateNewCopyMain() {
		...
		DocumentNode mainCurrentNode = TbUtil.createNewCopy(main,
		...
		assertEquals(VersionEnum.REFERENCE, main.getVersion());
		assertEquals(VersionEnum.CURRENT, mainCurrent.getVersion());
		assertEquals(1, main.getOtherVersions().size());
	...
	private void assertNewCopyResult(DocumentNode oldVersion) {
		...
		assertEquals(VersionEnum.REFERENCE, oldVersion.getVersion());
		assertEquals(1, oldVersion.getOtherVersions().size());

		DocumentNode newVersion = oldVersion.getOtherVersions().get(0);
		assertEquals(VersionEnum.CURRENT, newVersion.getVersion());
		...
		assertEquals(newVersion.isOffsetRelative(), oldVersion
				.isOffsetRelative());

		if (oldVersion instanceof TextBlock) {
			assertNewCopyResultBlock((TextBlock) oldVersion,
					(TextBlock) newVersion);
		} else {
			assertNewCopyResultToken((AbstractToken) oldVersion,
					(AbstractToken) newVersion);
		}
	}

	private void assertNewCopyResultBlock(TextBlock oldVersion,
			TextBlock newVersion) {
		if (oldVersion.getParentBlock() != null) {
			assertEquals(newVersion.getParentBlock(), oldVersion
					.getParentBlock().getOtherVersions().get(0));
		} else {
			assertEquals(null, newVersion.getParentBlock());
		}

		assertEquals(oldVersion.getSubBlocks().size(), newVersion
				.getSubBlocks().size());
		for (TextBlock oldSubBlock : oldVersion.getSubBlocks()) {
			assertEquals(true, newVersion.getSubBlocks().contains(
					oldSubBlock.getOtherVersions().get(0)));
		}

		assertEquals(oldVersion.getTokens().size(), newVersion.getTokens()
				.size());
		for (AbstractToken oldToken : oldVersion.getTokens()) {
			assertEquals(true, newVersion.getTokens().contains(
					oldToken.getOtherVersions().get(0)));
		}
	}

	private void assertNewCopyResultToken(AbstractToken oldVersion,
			AbstractToken newVersion) {
		assertEquals(newVersion.getParentBlock(), oldVersion.getParentBlock()
				.getOtherVersions().get(0));
	}
	...
	public void testReferenceVersions() {
		...
		assertEquals(true, main.getOtherVersions().contains(mainCurrent));
		assertEquals(true, mainPrevious.getOtherVersions().contains(main));
		assertEquals(true, mainPrevious.getOtherVersions()
				.contains(mainCurrent));
		assertEquals(true, mainCurrent.getOtherVersions().contains(main));
		assertEquals(true, mainCurrent.getOtherVersions()
				.contains(mainPrevious));
	...
	public void testFillNodeLevelMap() {
		Map<Integer, DocumentNode> nodeLevelMap = TbUtil.createNodeLevelMap(
				subToken, TbNavigationUtil.getLevel(subToken));
