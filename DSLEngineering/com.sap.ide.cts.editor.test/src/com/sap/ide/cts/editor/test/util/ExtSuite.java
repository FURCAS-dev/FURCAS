...
public class ExtSuite extends Suite {
	...
	private static Class<?>[] getAllClasses(Class<?> klass)
			throws InitializationError {
		HashSet<Class<?>> result = new HashSet<Class<?>>();
		result.addAll(getClassesInPackages(klass.getClassLoader(),
				getAnnotatedPackages(klass)));
	...
	private static List<Class<?>> getClassesInPackages(ClassLoader classLoader,
			String[] pckes) {
	...
	private static String[] getAnnotatedPackages(Class<?> klass)
			throws InitializationError {
		SuitePackages annotation = klass.getAnnotation(SuitePackages.class);
		if (annotation == null) {
			throw new InitializationError(String.format(
					"class '%s' must have a SuitePackages annotation", klass
							.getName()));
		}
		return annotation.value();
	}
	 ...
	 * @throws IOException
	 */
	private static List<Class<?>> getClassesInPackage(ClassLoader classLoader,
		...
		while (resources.hasMoreElements()) {
			...
			if (resource.getProtocol().equals("bundleresource")) {
				...
				System.out
						.println("ExtSuite is missing support for resource of protocol "
	...
	private static List<Class<?>> findClasses(File directory, String packageName)
		...
		for (File file : files) {
			if (file.isDirectory()) {
				...
				Class<?> candidate = Class.forName(packageName
						+ '.'
