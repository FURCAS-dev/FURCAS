...
//	 sourcePassThroughStdLibOpNames.add(PredefinedType.ATRPRE_NAME);
	...
	if (body != null) {
		...
		OCLExpression<EClassifier> argument = ((EList<OCLExpression<EClassifier>>) getExpression().getArgument())
		...
		if (argument instanceof TypeExp) {
		    ...
		    throw new RuntimeException("What else could be the argument of oclAsType if not a TypeExp? "
			    + ((EClass) argument.refMetaObject()).getName());
		}
		...
		NavigationStep sourcePath = pathCache.getOrCreateNavigationPath(getExpression()
			.getSource(), context, classScopeAnalyzer);
		if (argumentPassThroughStdLibOpNames.contains(opName)) {
		    OCLExpression<EClassifier> argument = ((JmiListImpl<OCLExpression<EClassifier>>) getExpression()
			    .getArguments()).get(getConnection().getSession(), 0);
		    NavigationStep argumentPath = pathCache.getOrCreateNavigationPath(argument, context, classScopeAnalyzer);
