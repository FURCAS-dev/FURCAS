...
public interface OperationBodyToCallMapper {
    ...
    /**
     * Determines the <code>self</code> variables as used by the operation body <code>body</code>. While it might seem a little
     * strange to have multiple <code>self</code> variables within the same operation body, that's however what may happen in
     * Ecore OCL. It would otherwise be tricky to identify the <code>self</code> variable when given the body or the
     * {@link EOperation} element. If <code>self</code> is not used by the body, then <code>null</code> will be returned. Never
     * returns <code>null</code> but may return an empty set.
     */
