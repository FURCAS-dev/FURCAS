...
public class PageableResourceDescriptorImpl implements ResourceDescriptorInternal {
	...
	public PageableResourceDescriptorImpl(URI _uri, long versionId, Map<String, String> uData, PagingResourceDescriptorMap<URI, PageableResourceDescriptorImpl> resTable,
			boolean pagedOut) {
		this.userData = uData;
	...
	private void serialize(SerializationStrategyFactory strategyFactory) {
		if (!this.isPagedOut()) { // FIXME this check should be done in the page
			...
			if (this.isIndexed()) {
				...
				this.outgoingLinkTable.serialize(strategyFactory.createOutgoingLinkMapStrategy(this.eObjectTable, this.resourceTable.getUnderlyingMap()));
			}
			this.incomingLinkTable.serialize(strategyFactory.createIncomingLinkMapStrategy(this, this.eObjectTable, this.resourceTable.getUnderlyingMap(), this.outgoingLinkTable));
	...
	public void serializeData(Channel channel) {
		if (this.userData == null) {
			channel.putInt(SerializationStrategyFactory.NO_INT); // FIXME
																	// constant
																	// position
			return;
		} else {
