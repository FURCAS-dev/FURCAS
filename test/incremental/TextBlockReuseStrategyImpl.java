...
/**
 * @see ITextBlockReuseStrategy
 * @author C5106462
 * 
 */
public class TextBlockReuseStrategyImpl implements ITextBlockReuseStrategy {

	private ITextBlockCreator tbCreator;
	private IReferenceHandler referenceHandler;
	private final Lexer lexer;
	private ShortPrettyPrinter shortPrettyPrinter;
	private List<PropertyArg> propertyArgs = new ArrayList<PropertyArg>();
	...
	private TbBean handleReferenceOnlyTemplate(TextBlock oldVersion, TextBlockProxy newVersion) {
		...
		// if (isTBEqual(oldVersion, newVersion)) {
		// int endIndex = 0;
		// if (TbNavigationUtil.isUltraRoot(oldVersion)
		// && !(newVersion.getSubNodes().get(0) instanceof Bostoken)) {
		// // need to add to index because ultra root has additinal
		...
		// .makeRelativeOffsetRecursively(oldVersion);
		//
		// TbValidationUtil.assertTextBlockConsistency(oldVersion);
		//
		// referenceHandler.setNewFeature(newFeatureBean,
		// TcsUtil.isReferenceOnly(subNodeResult.textBlock.getType().getParseRule()));
		// }
		// } else if (subNode instanceof AbstractToken) {
		// // if it is an additional token it has to be added to the
		// // reused textblock
		...
		// resolveProxies(oldVersion, newVersion);
		//	    
		// if(TcsUtil.isReferenceOnly(newVersion.getTemplate()) && valueChanged) {
		// //check if we need to re-query referenceOnly Templates
		// TextBlock tb = tbCreator.createNewTextBlock(newVersion,
		// oldVersion.getParentBlock());
		// if(!isEmpty(oldVersion)) {
		// //old version still there to this was an insert case
		// return new TbBean(tb, true,
		// ReuseType.INSERT, true);
		// } else {
		// //old version is empty now so this was a delete case
		// return new TbBean(tb, true,
		// ReuseType.DELETE, true);
		// }
		// }
		// return new TbBean(oldVersion, false, ReuseType.COMPLETE);
		// }
	...
	private boolean isEmpty(TextBlock oldVersion) {
...
//		Property propertyOfOldversion = (Property) oldVersion.getSequenceElement();
		...
		if (reference != null) {
			for (TextBlock tb : reference.getSubBlocks()) {
				...
				if (TbVersionUtil.getOtherVersion(tb, VersionEnum.CURRENT) == null) {
					...
					////////////////////////////
					///////////////////////////
					// guetter ici pourkoi  tb.getSequenceElement() instanceof Property et si c est meme important de use setNew... ici pour tcs_Property
